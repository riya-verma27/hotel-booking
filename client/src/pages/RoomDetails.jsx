...
const RoomDetails = () => {
    // to extract Id from Route Params
    const { id } = useParams();
    ...
    //on submit handler function to check availability & book the room
    ...
    return room &&(
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            ...
            <div className="mt-6 flex flex-col lg:flex-row gap-6">
                ...
                {/* Thumbnail Grid */}
                <div className="lg:w-[30%] w-full grid grid-cols-2 gap-4">
                    {room?.images.length > 1 &&
                        room.images.slice(0, 4).map((image, index) => (
                            <img
                                ...
                                src={image}
                                alt={`Thumbnail ${index}`}
                                className={`h-24 w-full rounded-lg object-cover cursor-pointer border-2 ${mainImage === image ? 'border-orange-500' : 'border-transparent'
                                    }`}
                            />
                        ))}

                    {/* Final wide image (if exists) */}
                    {room.images[4] && (
                        <img
                            onClick={() => setMainImage(room.images[4])}
                            src={room.images[4]}
                            alt="Thumbnail 4"
                            className={`col-span-2 h-24 w-full rounded-lg object-cover cursor-pointer border-2 ${mainImage === room.images[4] ? 'border-orange-500' : 'border-transparent'
                                }`}
                        />
                    )}
                </div>
                ...
                <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500'>
                    <div className='flex flex-col'>
                        ...
                        <input onChange={(e)=>setcheckInDate(e.target.value)} min={new Date().toISOString().split('T')[0]}
                         type="date" id='checkInDate' placeholder='Check-In' className='w-full rounded border 
                    ...
                    <div className='flex flex-col'>
                        ...
                        type="date" id='checkOutDate' placeholder='Check-Out' className='w-full rounded border 
                        border-gray-300 px-3 py-2 mt-1.5 outline-none' required />
                    </div>
                    <div className='w-px h-15 bg-gray-300/70 max-md:hidden'></div>
                    <div className='flex flex-col'>
                        <label htmlFor='guests' className='font-medium'>Guests</label>
                        <input onChange={(e)=>setGuests(e.target.value)} value ={guests} type="number" id='guests' placeholder='1' className='max-w-20 rounded border border-gray-300 px-3
            ...
            <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
                <p>Guests will be allocated on the ground floor according to the availability. You get a comfortable
                    two bedroom apartment that has a true city feeling. The price is quoted for two guests, at the
                    guest slot please mark the number of guests to get exact price for groups.
                </p>
            </div>
            {/* Hosted By */}
